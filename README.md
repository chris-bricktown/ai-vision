...
- **품종 확정 시 자동 정지 + 대표 이미지**: 품종 분류는 프레임마다 결과가 흔들릴 수 있어서(같은 피사체인데도 프레임마다 다른 품종으로 인식되는 경우), 신뢰도 임계값(기본 40%)을 넘는 인식 1회로 바로 확정하지 않고 **같은 품종이 누적 10회 인식되면** 실시간 인식을 멈추고 확정합니다(연속일 필요는 없음 — 중간에 다른 품종이 끼어들어도 그 품종의 누적치는 유지됩니다). 확정 전까지는 정보 패널 하단에 "품종 인식 현황"으로 그동안 인식된 품종들과 각각의 누적 횟수(예: "비글 7/10")를 실시간으로 보여줍니다. 확정되면 해당 품종의 대표 이미지를 [Wikipedia REST API](https://en.wikipedia.org/api/rest_v1/)에서 가져와 보여줍니다. "다시 인식하기" 버튼으로 재시작하면 누적 현황은 초기화됩니다. 품종 이름만으로 검색하면 엉뚱한 문서로 연결되는 경우가 있어서(예: "Havanese"는 개 품종이 아니라 쿠바 아바나시 문서로, "Boxer"/"Samoyed"는 동음이의어 문서로 연결됨) 먼저 "품종명 + dog/cat"으로 찾아보고, 그 문서가 없으면(예: "Golden Retriever"는 "Golden Retriever dog"라는 문서가 없음) 품종명 그대로 다시 찾습니다.
