...
- **줌**: 선택된 카메라가 [MediaTrackCapabilities의 `zoom`](https://developer.mozilla.org/en-US/docs/Web/API/MediaTrackCapabilities/zoom)을 지원하면(주로 Android Chrome) 줌 슬라이더가 나타나 하드웨어/드라이버 수준에서 배율을 조절할 수 있습니다. 지원하지 않는 기기(대부분의 노트북 웹캠, iOS Safari 등)에서는 슬라이더가 자동으로 숨겨집니다.
